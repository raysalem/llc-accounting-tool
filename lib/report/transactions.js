            ...
            try {
                ...
                if (catLower) {
                    ...
                    if (cData && cData.report === 'Transfer') {
                        // Warn if the transfer category is used on a sheet other than its Transfer Account
                ...
                console.error(`[ERROR] Sheet "${sheet.name}" Row ${r}: Crash detected. ${rowError.message}`);
                if (showDebug) console.error(rowError.stack);
                ctx.hasErrors = true;
            }
        });
