...
async function readSheetConfigs(ctx) {
    ...
    if (sheetConfigs.length === 0) {
        // Fallback defaults if no config found in Setup. offset: null lets
        // sheet-columns.js find the header row (row 1 by hand, row 3 after load_transactions.js).
        console.warn('[!] No sheet configurations found in Setup. Using defaults.');
        sheetConfigs.push({ name: 'Bank Transactions', shortName: 'Bank Transactions', type: 'Bank', flip: false, offset: null, linkedAccount: null });
        sheetConfigs.push({ name: 'Credit Card Transactions', shortName: 'Credit Card Transactions', type: 'CC', flip: true, offset: null, linkedAccount: null });
    }
}
